...
def getBoneTree(model):
    with open(model, "rb") as file:
        ...
        for bone in range(boneCount):
            ...
            print(bone, posX, posY, posZ, hex(bone_id), parent_bone if parent_bone != 0xFFFF else "Root")
            bones.append([posX, posY, posZ, bone_id, parent_bone])
    return bones


def drawNewBones(bones):
    ...
    for bone in bones:
        ...
        if bone[4] != 0xFFFF:
            ...
            ax.text(bone[0], bone[2], bone[1], "{}".format(hex(bone[3])), color="blue", zorder=0)
